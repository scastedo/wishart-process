...
def analysis(animal, sf, start,stop, array_conditions):
    ...
    # snr = np.nansum(overlaps_normal/eigs_normal)
    # SNR_OUTPUTS[-1] = (snr)
    ...
    for i, condition_number in enumerate(array_conditions):
        ...
        overlaps_super, eigs_super = calculate_overlap(mu_test_hat, sigma_test_hat, cos = False)
        snr_per_angle = np.nanmean(overlaps_super / eigs_super, axis=1)  # shape: (num_angles,)
        snr_mean = np.nanmean(snr_per_angle)
        SNR_OUTPUTS[i] = snr_mean
    return SNR_OUTPUTS
...
conditions_array = [12, 18, 36, 48, 72, 180]
...
np.savez('snr_results_sated_full_6_compare.npz', **snr_results)
