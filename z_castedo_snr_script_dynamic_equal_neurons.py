...
small_angle = [5.0,10.0,20.0,30.0]
N_SUBSAMPLE = 40
REPEATS = 10
...
SAVE_DIR = f"wishart_jun8_{'dynamic' if RUN_DYNAMIC else 'static'}_equal{N_SUBSAMPLE}_{DATASET_STATE}"
...
def validate_small_angles():
    if not 1 <= len(small_angle) <= 5:
        raise ValueError(f"small_angle must contain 1-5 degree gaps, got {small_angle}")
...
def analysis(
    ...
    latent_mean_orig_mode, sigma_orig_mode, _ = posterior.mode(x_full)
    signal_mean_orig = y_full.mean(axis=0)
    ...
    for angle_gap in small_angle:
        ...
        mu_test_hat_gap = gp.posterior_mode(x_full, signal_mean_orig, x_full_small)
    ...
    saved_summary = {
        ...
        "signal_mean_source": "empirical_condition_mean",
        "signal_interpolation": "periodic_gp_from_empirical_condition_mean",
        "mean_orig_mode": np.asarray(signal_mean_orig),
        "latent_mean_orig_mode": np.asarray(latent_mean_orig_mode),
